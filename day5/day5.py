...
def part2():
	...
	for seed_index in range(0, len(seeds), 2):
		...
		for value in range(seeds[seed_index], seeds[seed_index] + seeds[seed_index + 1]):
			for step in range(1, len(order)):
				i = lines.index(order[step - 1] + "-to-" + order[step] + " map:")
				while (i < len(lines) and len(lines[i]) and lines[i][0] in "0123456789"):
					map_range = [int(elem) for elem in lines[i].split(' ')]
					if (value >= map_range[1] and value < map_range[1] + map_range[2]):
						value = map_range[0] + (value - map_range[1])
						break
					i += 1
			if (min_location == -1 or value < min_location):
				min_location = value
				

	print(f'\n\nmin location found: {min_location}')
